...
此页面描述了 [包依赖管理](../package/description-scope.md) 的 `on_load()`、`on_install()` 或 `on_test()` 等函数的 `package` 接口
...
如果包是二进制的，则会返回 [`os.host`](../module/builtin-modules.md) 的值。

## package:arch

获取包的架构（例如 x86、x64、x86_64）。

如果包是二进制的，则返回 [`os.arch`](../module/builtin-modules.md)。
