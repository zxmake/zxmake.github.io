...
它将会在实际编译的时候，将内置的 `buildir` 变量转换为实际的构建输出目录：`-I./build`
...
一般默认是执行 `xmake` 命令时的工程根目录，当然如果通过 **os.cd** 改变了目录的话，这个值也会一起改变。
